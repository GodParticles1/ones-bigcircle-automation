...
async function ensureFieldReadTab(config) {
  ...
  const active = teamTabs.find((tab) => tab.active) || teamTabs[0];
  if (!active?.id) return { ok:false, status:"ONES_TAB_UNAVAILABLE", error:"无法定位 ONES team 页面" };
  return { ok:true, tabId:active.id, created:false, ready:{ href:active.url || null, teamUuid:scope.teamUuid } };
}
