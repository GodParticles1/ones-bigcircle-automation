...
foreach ($p in @($Cases,$Reconciliation,$FieldReads)) { if (-not (Test-Path -LiteralPath $p -PathType Leaf)) { throw ("INPUT_MISSING: " + $p) } }
...
python (Join-Path $Root "root-cause-sync\plan.py") --cases $Cases --reconciliation $Reconciliation --root-cause $Extraction --field-reads $FieldReads --field-id $FieldId --output $Plan
...
Write-Host ""
Write-Host "===== UNIQUE TASK PLAN ====="
Write-Host ("TASK_TARGET_POLICY=" + $PlanDoc.taskTargetPolicy)
Write-Host ("TASK_TARGET_COUNT=" + $PlanDoc.taskTargetCount)
Write-Host ("TASK_SET_CANDIDATE=" + $PlanDoc.taskTotals.SET_CANDIDATE)
Write-Host ("TASK_NOOP=" + $PlanDoc.taskTotals.NOOP)
Write-Host ("TASK_CONFLICT_REVIEW=" + $PlanDoc.taskTotals.CONFLICT_REVIEW)
Write-Host ("TASK_BLOCK=" + $PlanDoc.taskTotals.BLOCK)
