param(
  [string]$OldRelayDir = "D:\tools\ones-local-relay-v0.2.2",
...
if ($LASTEXITCODE -ne 0) { throw "RELAY_V030_START_FAILED" }

$health = Invoke-RestMethod -Method Get -Uri "http://127.0.0.1:18731/health" -TimeoutSec 5
if (-not $health.ok -or $health.version -ne "0.3.0") { throw "RELAY_V030_HEALTH_FAILED" }

Write-Host "RELAY_V030_UPGRADE_PASS"
